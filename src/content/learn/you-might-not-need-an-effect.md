...
Этот код удаляет лишнее **Воздействие** (Эффект) и исправляет беду.

### Отправка Наказа на Добавление (POST-запроса) {/*sending-a-post-request*/}

Представим, что часть страницы `Form` (Форма) делает два вида наказов на добавление. Она отправляет весть для учёта (аналитики) при рождении. А когда ты заполняешь бланки и нажимаешь кнопку «Отправить», она пошлёт наказ на добавление по адресу `/api/register`:
...
Однако ты быстро обнаружишь, что **Воздействие** (Эффект) [исполняется дважды в режиме разработки.](/learn/synchronizing-with-effects#how-to-handle-the-effect-firing-twice-in-development) Это может создать беды — например, возможно, это сделает негодной печать доступа (токен аутентификации), потому что само дело не было рассчитано на двойной призыв. В целом, части страницы должны быть устойчивы к повторному рождению. Это касается и самой главной части — `App` (Приложение).

Хотя на деле она может никогда не родиться заново в чистовой версии (в миру), соблюдение одних и тех же правил во всех частях упрощает их перенос и повторное использование кода. Если некое правило должно исполниться *один раз при запуске всего дела*, а не *один раз при рождении отдельной части*, добавь переменную самого верхнего уровня, чтобы знать, было ли это дело уже исполнено:

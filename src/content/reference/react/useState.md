...
`useState` — это **подключение** Расписывателя, которое дозволяет тебе наделить свою [составную часть](/learn/state-a-components-memory) **хранимкой** (переменной памятью).
...
Зови `useState` (установить хранимку) в самом начале своей составной части. Таким способом, ты заведёшь **хранимку** [для её вестей.](/learn/state-a-components-memory)
...
Обычай велит давать имена **хранимкам** по образцу `[something, setSomething]`, используя [расплетание ряда.](https://javascript.info)

[Гляди примеры ниже.](#usage)

#### Передаваемые значения  {/*parameters*/}

* `initialState`(начальная хранимка): Значение, которое ты хочешь заложить в **хранимку** изначально. Оно может быть любого рода, но для действий (функций) есть особый нрав. Это  значение будет оставлено без внимания после первой отрисовки.
  * Если ты передашь действие как `initialState`, оно будет считаться _первичным деянием_. Оно должно быть чистым, не принимать  значения  и возвращать значение любого рода. Расписыватель позовёт твоё начальное действие при создании составной части и вернёт его исход как начало **хранимки**. [Гляди пример ниже.](#avoiding-recreating-the-initial-state)

#### Исход {/*returns*/}

`useState` возвращает ряд ровно из двух значений:

1. Нынешнюю **хранимку**. При первой отрисовке оно будет ладить с тем `initialState`, что ты передал.
2. [указ `set`](#setstate) (указ-правильщик), которое дозволяет тебе обновить **хранимку** до иного значения и побудить к **перерисовке**.

#### Предостережения {/*caveats*/}

* `useState` — это **Подключение**, посему ты можешь звать его только **в самом вверху(начале) своей составной части** или в своих собственных Подключениях. Ты не можешь звать его внутри кругов (циклов) или условий. Если тебе это надобно — выдели новую часть и перенеси **хранимку** в неё.
* В **Строгом уставе** Расписыватель **позовёт твоё начальное деяние дважды**, дабы [помочь тебе сыскать случайную нечистоту в уставе.](#my-initializer-or-updater-function-runs-twice) Сие творится лишь при создании и не влияет на **живое деяние**. Если твоё дело чистое (каким ему и подобает быть), это не сдвинет смысл работы. Исход одного из призывов будет отброшен.

---

### Указы `set` , вроде `setSomething(nextState)` {/*setstate*/}

Указ-правильщик `set`, возвращаемое `useState`, дозволяет тебе обновить **хранимку** до иного значения и побудить к **перерисовке**. Ты можешь передать новую хранимку прямо или через действие, которое высчитает его на основе прошлого:
...
#### Передаваемые значения  {/*setstate-parameters*/}

* `nextState(следующая хранимка)`: Значение, которое ты хочешь заложить в **хранимку**. Оно может быть любого рода, но для деяний есть особый нрав.
  * Если ты передашь деяние как `nextState`, оно будет считаться _обновляющим деянием_. Оно должно быть чистым, принимать лишь одно  значение (ожидаемую хранимку) и возвращать следующую хранимку. Расписыватель поставит твоё обновляющее деяние в очередь и наметит **перерисовку** части. Во время следующей отрисовки Расписыватель высчитает новую хранимку, применив все накопленные в очереди деяния к прошлому значению. [Гляди пример ниже.](#updating-state-based-on-the-previous-state)

#### Исход {/*setstate-returns*/}

Указы `set` не возвращают никакого значения.

#### Предостережения {/*setstate-caveats*/}

* Указ `set` **обновляет хранимку только для *следующей* отрисовки**. Если ты попытаешься прочесть значение из **хранимки** тотчас после призыва `set`, [ты всё ещё получишь старое значение](#ive-updated-the-state-but-logging-gives-me-the-old-value), которое было на экране до твоего призыва.
...
Зови `useState` в самом верху своей составной части, дабы объявить одну или несколько [хранимок.](/learn/state-a-components-memory)
...
1. <CodeStep step={1}>Нынешнее состояние</CodeStep> этой **хранимки**, изначально равное тому <CodeStep step={3}>началу</CodeStep>, что ты предоставил.
2. <CodeStep step={2}>указ `set`</CodeStep> (указ-правильщик), которое дозволяет тебе сменить его на любое иное значение в ответ на волеизъявление.

Чтобы обновить то, что начертано на экране, позови указ `set` с новым состоянием:
...
Призыв указа `set` [**не меняет** нынешнюю **хранимку** в уже выполняемом уставе](#ive-updated-the-state-but-logging-gives-me-the-old-value):
...
По обычаю, ожидаемое значение принято величать первой буквицей имени **хранимки**, например `a` для `age`. Однако ты можешь прозвать его и `prevAge` или как-то иначе, если тебе так понятнее.

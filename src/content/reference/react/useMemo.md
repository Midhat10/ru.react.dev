...
* `calculateValue`: Дело (функция), возвращающее значение, которое нужно положить в **запасник** (закешировать). Оно должно быть чистым и не должно принимать никаких доводов (аргументов). **Расписыватель** (React) обязательно призовёт его при первой отрисовке, а при последующих будет возвращать готовое значение из запасника. В том случае если какое-то значение из перечня условий `dependencies` изменилось, **Расписыватель** заново призовёт `calculateValue`, получит новую добычу и сохранит её для будущего использования.

* `dependencies`: Перечень (массив) всех **чутких значений**, которые используются внутри дела `calculateValue`. Чуткие значения включают в себя входящие данные (пропсы), данные из хранилищ (состояние) и все переменные и дела, объявленные внутри части страницы. Если ты настроил своего **надзирателя за кодом** (линтера), он проверит, все ли чуткие части были указаны как условия. Перечень условий должен иметь конечное число звеньев и быть описан как `[условие1, условие2, условие3]`. **Расписыватель** будет сравнивать каждое такое условие с его прежним значением при помощи [`Object.is`](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/is).

#### Возвращаемое значение {/*returns*/}

При первой отрисовке `useMemo` (запомнить значение) возвращает плод призыва дела `calculateValue`.

При всех последующих отрисовках `useMemo` либо будет возвращать значение, сохранённое в запаснике при прошлой отрисовке (если ничто в перечне условий не изменилось), либо заново призовёт дело `calculateValue` и вернёт его новую добычу.

#### Предостережения {/*caveats*/}

* Поскольку `useMemo` — это особый прием (хук), то призывать его можно только **на самом верхнем уровне внутри твоей части страницы** или самодельного приема. Приемы нельзя призывать внутри кругов (циклов) или условий. Однако, если тебе необходимо такое поведение, то можно выделить новую часть страницы и перенести призыв приема в неё.
* В **Строгом Укладе** **Расписыватель** дважды призовёт переданное в `useMemo` дело, чтобы проверить, [является ли оно чистым](#my-calculation-runs-twice-on-every-re-render). Такое поведение существует только в режиме разработки и никак не проявляется в миру (продакшене). Если это дело чистое (каким оно и должно быть), это никак не скажется на работе. Плод одного из запусков будет просто отброшен.
* **Расписыватель** **не выкидывает значение из запасника, если на то нет веских причин**. Например, в режиме разработки он опустошит запасник, если файл части страницы был изменён. И в режиме разработки, и в миру **Расписыватель** сбросит запасник, если часть страницы [«замедлится»](/reference/react/Suspense) во время самого первого рождения. Кроме того, в будущем могут быть добавлены новые причины для сброса запасника. Полагаться на `useMemo` стоит только как на средство для **ускорения работы** (оптимизации). В ином случае использование [хранилища](/reference/react/useState#avoiding-recreating-the-initial-state) или [верной ссылки](/reference/react/useRef#avoiding-recreating-the-ref-contents) может быть более подходящим.


<Note>

Подобное хранение добытых значений в запаснике называют [*мемоизацией*](https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D0%BC%D0%BE%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F) (запоминанием), посему данный прием (хук) и был величан `useMemo` (запомнить значение).
...
### Пропуск тяжёлых пересчётов {/*skipping-expensive-recalculations*/}

Чтобы положить плоды вычислений в **запасник** (закешировать) между повторными отрисовками, достаточно обернуть их в `useMemo` (запомнить значение) на самом верхнем уровне внутри твоей части страницы.
